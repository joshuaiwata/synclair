...
Each tier is a **gallery** of card previews — live thumbnails on a dot-grid stage —
and each item has its own **doc page** (`/synclair/{tier}/[name]`) with:

- **A preview stage** — the real component rendering (not a screenshot) on a
  Storybook-style canvas: a device-width switcher on every tier (components get
  desktop / 768 / 375; blocks and templates add wide and fullscreen), a stage
  light/dark flip that previews the opposite theme without touching the page, and a
  **Code** toggle with one-click copy. (In existing-app mode, host components are
  documented from their source and tagged **"Host"**.)
...
## What a doc page gives you

Open any item and you get the documentation a design system actually needs — intent,
anatomy, interaction rules, responsive behavior, the real API — beside a live preview you
can flip between viewports and themes.

<Frame caption="A block's doc page. Depth scales with tier: a component needs less than a block, a block less than a template — and docs are commit-anchored to the source they describe, so drift is flagged instead of quietly accumulating.">
  <img src="/images/component-doc.png" alt="A block's doc page showing a live preview with viewport and theme switchers, its dependencies, and its UX documentation" />
</Frame>
