...
<CardGroup cols={2}>
  ...
  <Card title="Foundation freshness" icon="arrows-rotate">
    Whether this clone is behind the mother Synclair repo — the opt-in "call home" check.
  </Card>
</CardGroup>
...
- **The Synclair view** — the foundation freshness state: your clone's **baseline** (the
  mother sha it last synced to) and, when call-home is on, how many foundation updates
  are available with their commit titles.

## How it works

Freshness uses an **opt-in "call home"** seam (`data/mother.json`). When enabled, the hub
compares your recorded baseline against the mother repo via GitHub's public compare API
...
<AccordionGroup>
  ...
  <Accordion title="Promote an improvement upstream" icon="arrow-up">
    ...
    into the mother repo — the flywheel that improves Synclair for every clone.
  </Accordion>
</AccordionGroup>
