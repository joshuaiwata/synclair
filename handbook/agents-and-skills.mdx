...
<Frame caption="/synclair/ai-setup reads .claude/ live, so the crew on the page is the crew on disk. Every capability is tagged foundation (ships with Synclair, syncs from upstream) or project (yours, never syncs).">
  <img src="/images/ai-setup.png" alt="The AI Setup page listing the skills that ship with the clone, each with its origin and a one-line description of when it applies" />
</Frame>
...
but Synclair's skills live in the `synclair/` subdir — a place agent tools don't scan. So
...
ambiently, without cd-ing into `synclair/`. Re-run the bridge after adding an ambient skill.
</Note>
...
Skills and agents are *know-how*. **MCP servers are reach** — the connections that let an
agent do something outside the repo: pull a frame from Figma, read a PRD in Notion, drive a
browser to verify a view.

Synclair works both ends of this. It **is** an MCP server — nine read-only tools over the
hub's own data — *and* it catalogs the servers your project is wired to.

### Synclair's own MCP server

Ambient markdown is how an agent learns *how you work*; it's a poor way to ask one narrow
question. So the same data is also served as tools:
...
- **No hub required** — it reads JSON off disk, so there's no port, no dev server, and
  nothing to collide with the hub on 4100.
- **No dependencies** — raw JSON-RPC over stdio rather than the SDK, so it works in any
  clone with zero install.
...
Register it with `npm run mcp:install`. It's opt-in by design and deliberately not wired to
`postinstall` — writing a repo's own `.mcp.json` is fair, but writing your *global* agent
config as an install side effect would be invisible and hard to undo.

<Note>
**What it saves, stated carefully:** ~40% against equivalent file reads across comparable
scenarios, versus a permanent +854-token tool surface. An earlier version of our own
measurement harness claimed 86–97%; it was wrong three ways (skill bodies counted as lookup
cost, filtered queries compared against whole-file reads, blank seed data producing fake
wins). Fixed and enforced in the script, which now prints *"not comparable"* rather than a
flattering number.
</Note>

### The servers your project is wired to

Separately, Synclair catalogs the *other* servers your agents can reach, on the MCP tab of
`/synclair/ai-setup` — each with its connection status and, more usefully, *what it's for
here*:

| Server | Its role in this project |
|---|---|
| **Figma** | Design context, screenshots, variables, Code Connect, write-back. The primary design bridge |
| **Notion** | Where PRDs live, once their home is confirmed |
| **Supabase** | Available if the prototype needs real data |
| **Chrome / Preview** | Browser control for visual verification of views |
...
Edit the list in `lib/synclair-data.ts`; the connections themselves are configured in your
...
It spins from day one: intake **distills every source as it's linked** (an undigested
manifest entry is unfinished work), and when a later dig surfaces something durable,
the digger **writes it back** into the relevant skill. So the expensive read happens
once, up front — every read after is a cheap one-line load. Over time the distilled
brain converges on exactly what a builder routinely needs, and raw sources get
consulted only for genuinely new detail.
...
| Building anywhere in the product app (embedded mode) | **`building-in-this-repo`** |
