...
For the narrow task of *"see my components rendered in a gallery,"* they overlap — and
Synclair's doc pages deliberately borrow the good parts of Storybook's canvas: a
dot-grid stage with a device-width switcher, a stage light/dark flip, and a code
toggle with copy. If a component workshop is genuinely all you need, Storybook is the
more focused, mature tool and you should use it.
...
## And against agent-context tools?

A newer category — repowise, CodeGraph, Serena and friends — also promises to stop your
agent starting from zero. Different job, and a genuinely complementary one.

| | **Agent-context tools** | **Synclair** |
|---|---|---|
| **What they index** | The code that already exists | The decisions behind what gets written |
| **Typical output** | Call graph, git archaeology, dead code, code health | Tokens, component conventions, product requirements, maps |
| **Read or write** | Helps an agent **read** your codebase well | Governs what an agent **writes** into it |
| **How context reaches the agent** | An MCP server, queried per task | **Both** — ambient markdown *and* an MCP server with nine read-only tools |
