...
- **Before you catalog it at all: is it a REGION of something you're also cataloging?** The tier rubric decides *which* tier an entry belongs in; it never asks whether the entry should exist beside its own container. So a screen, the pane inside it, and the rail inside that all pass the rubric individually and all get written — and the gallery then shows three cards whose thumbnails contain one another. The reader has no way to see the nesting and reads it as duplication. This is the single largest source of catalog bloat observed in practice: a real clone reached 108 blocks where ~46 were distinct. **Resolve containment before writing.** Build the import graph of your candidate set first, and for any candidate whose render is dominated by another candidate, or that is composed only inside one other candidate, catalog the container and let the region live inside it — its screenshot already shows the region. Catalog a region separately only when something *outside* its parent composes it too. `npm run check:tiering` reports exactly this (wrappers, parts, and the full nesting chains) and is the check to run against your proposed set BEFORE writing entries, not after.
