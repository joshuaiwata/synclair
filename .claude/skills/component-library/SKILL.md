...
- **One piece of UI, one entry — catalog the reusable link, not every level of the composition.** "Register on creation" is about *extraction*; it is not licence to register a wrapper AND the thing it wraps AND the thing inside that. When a screen, the pane it renders, and the drawer inside the pane are all cataloged as peers, the gallery shows what looks like three similar blocks; the app has one, photographed at three zoom levels. That is how a ~20-block library reads as 108, and it is a cataloging fault rather than a component fault — the reader cannot tell the difference from the cards. Two questions settle it, in order: **is this entry's render dominated by one other cataloged entry?** (then it documents its child — catalog the child), and **is it composed anywhere other than that one parent?** (if not, it is a part of that parent, and the page that owns it is already recorded in `/synclair/pages`). Run `npm run check:tiering`: it derives the containment graph from real imports and prints the wrappers, the parts, and the nesting chains whole, so the "which link is the reusable one?" call is made with the shape visible instead of one pair at a time. Advisory by design — it has no opinion about which link deserves the tier, only that documenting every link is how the shelf fills up with the same picture.
