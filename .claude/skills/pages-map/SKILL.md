...
2. **Send the `page-mapper` agent** (its own context) at the target. It enumerates routes and returns, per page: `route`, `id`, `file`, `kind`, `title`, `summary`, `links`, `previewUrl`/`previewable`, and (multi-surface only) `surface`. It does **not** hand-resolve `items`/`sourceFiles`/`sourceHash` — those are computed deterministically in steps 4–5. For a **host** target, make sure `repo.previewBaseUrl` is set to the host dev server origin so the previews frame the real host routes — and keep the matching `data/dev-servers.json` entry on the SAME origin (the preview panel resolves through it). Beware the silent variant of this drift: a server that IS live on the recorded origin but running a different checkout/build frames the wrong app — new routes 404 in previews while every machine check stays green. When exploring a branch on an alternate port, update both files (and revert before committing).
