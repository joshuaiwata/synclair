...
| **Knowledge freshness** (has an upstream PRD/spec/deck moved since its digest was written? Probes each *linked* source's real last-modified vs. `distilledAt`, `npm run check:knowledge` — the source-side twin of call-home; Figma freshness is tracked in-app via the Manifest; re-distill via `product-spec`/`figma-distiller`) | `data/knowledge/freshness.json` (mechanism: `lib/system/knowledge/freshness.ts`; queue: `data/knowledge/redistill-queue.json`) | `/synclair/knowledge` |
...
| **Local dev servers** (companion mode — the host front-end(s)' local URLs + boot commands, so the hub lights up host route previews when they're running and offers to boot them when not; `npm run host:status` / `npm run host:up`) | `data/dev-servers.json` (mechanism: `lib/system/dev-servers.ts`) | live/boot banner on `/synclair/pages` |
...
<!-- SYNCLAIR:STATUS:START -->

## This clone, right now

_Generated by `npm run gen:agents-block` — anything between the markers is overwritten. Counts are what THIS clone has actually been populated with; zero means not done yet, not unavailable._

- **Product** — not named yet (`lib/system/seed/project.ts`)
- **Setup** — unset
- **Library** — 20 components · 9 blocks · 0 templates (30 native, 0 host)
- **Pages** — not mapped yet (`pages-map` skill)
- **System map** — not generated yet (`codebase-map` skill)
- **Knowledge** — no sources registered yet

Agents: prefer the `synclair` MCP tools over reading these files — `get_overview` answers most of the above in one call.
